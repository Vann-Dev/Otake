...
module.exports.execute = async(bot, msg, args, data) => {
    ...
    if(!newPrefix.length) {
        let embed = new Discord.MessageEmbed()
            ...
            .setDescription('My prefix for this guild is `' + prefix + '`, Wanna change prefix? type `' + prefix + 'prefix [new prefix]`')
    ...
    if(newPrefix.length > 5) {
        let embed = new Discord.MessageEmbed()
            .setColor(bot.config.color)
            .setDescription('Prefix shouldn\'t be longer than 5 characters. Yours has ' + newPrefix.length + '.')
        return msg.channel.send(embed);
    }
